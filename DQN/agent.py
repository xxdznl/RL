...
class ReplayMemory:
    ...
    def sample(self):
        ...
        batch_state_tensor = torch.as_tensor(np.asarray(batch_state), dtype=torch.float64)
        ...
        batch_reward_tensor = torch.as_tensor(np.asarray(batch_reward), dtype=torch.float64).unsqueeze(-1)
        batch_state_next_tensor = torch.as_tensor(np.asarray(batch_state_next), dtype=torch.float64)
...
class Agent:
    def __init__(self, n_state, n_action, learning_rate, gamma, memory_size, batch_size) -> None:
        ...
        self.memory_size = memory_size
        self.batch_size = batch_size
        self.memory = ReplayMemory(n_state=self.n_state, n_action=self.n_action, memory_size=self.memory_size,
